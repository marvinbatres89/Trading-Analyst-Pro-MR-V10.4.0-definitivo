...
function renderLanguage() {
  ...
  const translate = (element) => {
    if (!element) return;
    const original = element.dataset.rawValue || element.textContent;
    element.dataset.rawValue = original;
    element.textContent = i18n.translateState(original);
  };

  [UI.trend, UI.momentum, UI.volatility].forEach(translate);
}
