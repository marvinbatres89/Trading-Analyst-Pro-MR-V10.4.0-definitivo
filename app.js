...
async function runCountdown(seconds) {
  ...
  for (let remaining = seconds; remaining >= 0; remaining -= 1) {
    ...
    await voiceAssistant.speak(String(remaining), {
      replace: true,
      rate: Math.min(0.9, voiceAssistant.rate)
    });
...
async function beginPredictionSequence(result) {
  ...
  setText(UI.engineStage, "PREPARANDO OPERACIÓN");
  setText(UI.engineDetail, "Escuche la explicación y prepare el bot.");
  UI.engineProgress.style.width = "72%";
  showFloating(
    "prepare",
    "PREPARE",
    visualDirection(result),
    explanation || "Filtros superados."
  );

  await voiceAssistant.announcePrediction(result, explanation);
  await sleep(ENGINE.preparePauseMs);
  ...
  setText(
    UI.engineDetail,
    recommendation.second
      ? `Momento observado: segundo ${recommendation.second}. Ejecute dentro de este único conteo.`
      : "Ejecute manualmente dentro de este único conteo."
  );
  UI.engineProgress.style.width = "100%";
  showFloating(
    "confirmed",
