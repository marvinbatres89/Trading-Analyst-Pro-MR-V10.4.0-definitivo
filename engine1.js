...
function withSnapshotMark(result, snapshot) {
  return {
    ...
    metadata: {
      ...(result.metadata || {}),
      snapshotMark: snapshot?.snapshotMark ? { ...snapshot.snapshotMark } : null
    }
  };
}
...
function riseFall(snapshot) {
  ...
  if (snapshot.alignment?.dominant === "BULLISH" && snapshot.alignment?.agreement >= 4) {
    bullish += 6;
    upReasons.push("Alineación amplia de indicadores.");
  }
  if (snapshot.alignment?.dominant === "BEARISH" && snapshot.alignment?.agreement >= 4) {
    bearish += 6;
  ...
  const approvedCandidate = difference >= 12 && !snapshot.lateral && snapshot.volatility.level !== "VERY HIGH" && !snapshot.alignment?.conflicting;

  return output(
    "rise_fall",
    approvedCandidate ? direction : "WAIT",
    approvedCandidate ? score : score - 10,
...
function digitBinary(strategy, snapshot) {
  ...
  if (short.count < 20 || medium.count < 40 || long.count < 80) {
    return output(strategy, "WAIT", 0, [
      `Recopilando dígitos: ${short.count}/20 · ${medium.count}/40 · ${long.count}/80.`
    ]);
  }
  ...
  const agreement = 1 + Number(shortSideA === mediumSideA) + Number(shortSideA === longSideA);
  const stable = agreement === 3 && shortDiff >= 10 && mediumDiff >= 6 && longDiff >= 4;

  let score = 48 + Math.min(22, shortDiff * 0.9) + Math.min(12, mediumDiff * 0.45) + Math.min(7, longDiff * 0.2) + (agreement === 3 ? 8 : agreement === 2 ? 2 : -8);
  if (shortDiff < 8) score -= 12;
  ...
  return output(
    ...
    { shortDiff, mediumDiff, longDiff, agreement, stable }
  );
}
...
function spikeWatch(strategy, snapshot) {
  ...
  if (prices.length < 45) {
    return output(strategy, "WAIT", 0, [`Recopilando precios para ${strategy === "boom" ? "Boom" : "Crash"}: ${prices.length}/45.`], ["Motor experimental de contexto previo a picos."]);
  }
  ...
  const driftMatches = strategy === "boom" ? ["BEARISH", "LATERAL"].includes(snapshot.trend.direction) : ["BULLISH", "LATERAL"].includes(snapshot.trend.direction);
  const momentumMatches = strategy === "boom" ? ["NEGATIVE", "NEUTRAL"].includes(snapshot.momentum.direction) : ["POSITIVE", "NEUTRAL"].includes(snapshot.momentum.direction);
  const flowMatches = strategy === "boom" ? ["BEARISH", "NEUTRAL"].includes(snapshot.shortFlow.direction) : ["BULLISH", "NEUTRAL"].includes(snapshot.shortFlow.direction);
  ...
  let score = 48;
  const reasons = [];
  const warnings = ["Boom/Crash es experimental: el momento exacto del pico no puede garantizarse.", "La señal evalúa contexto reciente, no certeza del siguiente tick."];
  ...
  if (ticksSinceSpike >= 6) { score += 9; reasons.push(`Sin pico dominante en los últimos ${ticksSinceSpike} ticks observados.`); }
  else { score -= 16; warnings.push("Se detectó un pico reciente; se aplica enfriamiento."); }
  if (stableRun >= 7) { score += 7; reasons.push(`Secuencia reciente consistente: ${stableRun}/12 movimientos compatibles.`); }

  score = clamp(score, 0, 100);
  const approved = driftMatches && momentumMatches && flowMatches && ticksSinceSpike >= 5 && score >= 76;

  return output(strategy, approved ? (strategy === "boom" ? "BOOM" : "CRASH") : "WAIT", score, reasons.length ? reasons : ["Contexto todavía insuficiente."], warnings, { ticksSinceSpike, typicalMovement: typical, spikeLimit, stableRun });
}
...
function matches(snapshot) {
  ...
  if (short.count < 20 || medium.count < 40 || long.count < 80) {
    return output("match", "WAIT", 0, [`Recopilando dígitos para Matches: ${short.count}/20 · ${medium.count}/40 · ${long.count}/80.`]);
  }
  ...
  if (first.digit === 0) {
    return output("match", "NO_OPERAR", 0, ["El número 0 es el candidato dominante."], ["Regla del proyecto: 0 = NO OPERAR."], { digit: 0, separation });
  }

  const leaderVotes = [short.hotDigit, medium.hotDigit, long.hotDigit].filter((digit) => digit === first.digit).length;
  const recentHits = recentFrequency[first.digit] || 0;
  const support = leaderVotes === 3 && short.hotDigit === first.digit && recentHits >= 2;

  let score = 52 + Math.min(14, separation * 6) + Math.min(12, Math.max(0, first.score - 9.5) * 2.5) + (leaderVotes === 3 ? 9 : 0) + (recentHits >= 2 ? 5 : 0);
  score = Math.min(90, clamp(score, 0, 100));
  const approved = support && separation >= 1.0 && first.score >= 10.8 && score >= 80;

  return output("match", approved ? "MATCH" : "WAIT", score, [
  ...
  ], ["Matches sigue siendo experimental.", "La frecuencia pasada no garantiza el siguiente dígito."], { digit: first.digit, separation, combinedFrequency: first.score, leaderVotes, recentHits, support });
}
...
export function exploreOpportunity(strategy, snapshot) {
  ...
  let result;
  if (strategy === "rise_fall") result = riseFall(snapshot);
  else if (strategy === "even_odd") result = digitBinary("even_odd", snapshot);
  else if (strategy === "over_under") result = digitBinary("over_under", snapshot);
  else if (strategy === "boom" || strategy === "crash") result = spikeWatch(strategy, snapshot);
  else result = matches(snapshot);
