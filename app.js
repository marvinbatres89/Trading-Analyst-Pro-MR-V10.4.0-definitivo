...
[
  ...
  "entryAlertEnabled","entryAlertSecond","entryFlash"
].forEach((id) => { UI[id] = $(id); });
  ...
  Object.entries(markets)
    ...
    .sort((a, b) => {
      const rank = ([symbol, market]) => {
        ...
        const n = Number((name.match(/(?:Volatility\s+)(\d+)/i) || [0, 999])[1]);
  ...
  return new Promise((resolve) => {
    ...
    const update = () => {
      ...
      if (remaining !== lastShown) {
        ...
        const target = Number(UI.entryAlertSecond?.value || 7);
        if (UI.entryAlertEnabled?.checked && remaining === target && UI.entryFlash) {
          ...
          setTimeout(() => { UI.entryFlash.hidden = true; }, 700);
        }
      }
      if (elapsed >= seconds) {
        clearInterval(state.countdownTimer);
...
derivAPI.on("activeSymbols", ({ items }) => {
  ...
  setText(
    ...
    `${items.length} símbolos recibidos; se mostraron los mercados compatibles.`
  );
});
