...
function finishPrediction(message) {
  ...
  voiceAssistant.speak("Predicción finalizada. Puede buscar una nueva señal.");
...
function beginCountdown(result) {
  let remaining = ENGINE.countdownSeconds;
  setText(UI.countdown, remaining);

  voiceAssistant.speak("Tiene diez segundos para ejecutar la operación.");
  ...
  state.countdownTimer = setInterval(() => {
    ...
    if (remaining > 0) {
      voiceAssistant.speak(String(remaining), false);
    }
...
async function requestPrediction() {
  ...
  await new Promise((resolve) => setTimeout(resolve, ENGINE.responseDelayMs));
