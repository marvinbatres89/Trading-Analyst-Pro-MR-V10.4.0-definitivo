...
function matches(snapshot) {
  ...
  if (short.count < 24 || medium.count < 40) {
    return output("match", "WAIT", 0, ["Recopilando más dígitos para Matches."]);
  }
  ...
  for (let digit = 0; digit <= 9; digit += 1) {
    scores[digit] =
      (short.frequency[digit] / Math.max(1, short.count)) * 50 +
      (medium.frequency[digit] / Math.max(1, medium.count)) * 32 +
      (long.frequency[digit] / Math.max(1, long.count)) * 18;
  }
  ...
  if (first.digit === 0) {
    return output(
      "match","NO_OPERAR",0,
      ["El número 0 domina las ventanas recientes."],
  ...
  const sameLeader = leaderVotes >= 2;

  const score =
    50 +
    (sameLeader ? 12 : 0) +
    Math.min(18, separation * 6) +
    Math.min(22, Math.max(0, first.score - 10) * 2.5);
  ...
  return output(
    ...
    score,
    [
      `El número ${first.digit} lidera las ventanas corta y media.`,
      `Separación frente al segundo candidato: ${separation.toFixed(2)} puntos.`,
      `Frecuencia combinada: ${first.score.toFixed(2)}%.`
    ],
    [
      "Matches es experimental y exige mayor calidad.",
      "La frecuencia pasada no garantiza el siguiente dígito."
    ],
    { digit: first.digit, separation, combinedFrequency: first.score, sameLeader, leaderVotes }
  );
}
